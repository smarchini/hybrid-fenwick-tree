...
def plot(infile, outfile):
    with open(infile, "r") as csvfile:
        ...
        ydata6 = [ float(elements[i][5]) for i in range(1, len(elements)) ]
        fig = plt.figure()
        ax = fig.add_subplot(1,1,1)
        ax.semilogx(xdata, ydata1, label='simple')
        ax.semilogx(xdata, ydata2, label='typed')
        ax.semilogx(xdata, ydata3, label='ityped')
        ax.semilogx(xdata, ydata5, label='byte')
        ax.semilogx(xdata, ydata6, label='compact')
        ax.semilogx(xdata, ydata6, label='shrank')
