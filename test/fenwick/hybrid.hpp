...
template <typename T> void fenwick_random_test(std::size_t size) {
  ...
  for (size_t i = 1; i <= size; i++)
    EXPECT_EQ(naive.prefix(i), mixed.prefix(i))
        << "At index: " << i << "\nsize: " << size << "\ntemplate argument: " << typeid(T).name();

  // find
  for (std::uint64_t i = 0; i < size; i++)
    EXPECT_EQ(naive.find(i), mixed.find(i))
        << "At index: " << i << "\nsize: " << size << "\ntemplate argument: " << typeid(T).name();
  ...
  // find complement
  for (std::uint64_t i = 0; i < size; i++)
    EXPECT_EQ(naive.compFind(i), mixed.compFind(i))
        << "At index: " << i << "\nsize: " << size << "\ntemplate argument: " << typeid(T).name();
