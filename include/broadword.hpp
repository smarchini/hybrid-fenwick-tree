 ...
 * Petri [2] and Vigna [3]. Implementation from Facebook's folly library [4].
 *
 ...
 */
inline std::uint64_t select64(uint64_t x, uint64_t k)
{
// TODO: vedere se esiste una macro predefinita. se non esiste, provare ad ottenerala automaticamente con il makefile
#ifndef CPU_INTEL_HASWELL
    //DCHECK_LT(k, popcount(x)); // Serve? forse no, visto che la select la uso solo dopo aver usato la find
